  ...
  create: function(context) {
    ...
    return {
      ClassBody: (node) => {
        ...
        if (instanceMethodsWhichMustBeBound.length) { // If there are any instance methods
          const constructor = extractConstructorMethodFromBodyNode(node);
          if (!constructor) {
            context.report(node, noConstructorErrorText);
            return;
          }
          ...
          instanceMethodsWhichMustBeBound
            .forEach(instanceMethod => // Verify each remaining identifier is bound
              ...
              context.report(instanceMethod, methodNotBoundInConstructorErrorText(instanceMethod.key.name))
            );
        }
      }
    };
