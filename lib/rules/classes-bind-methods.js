...
const extractIgnoreSubclassesFromContext = context =>
  extractOptionsFromContext(context).ignoreSubclasses;

const extractOnlySubclassesFromContext = context =>
  extractOptionsFromContext(context).onlySubclasses;

const extractIgnoreFileExtensionsFromContext = context =>
  extractOptionsFromContext(context).ignoreFileExtensions;

const extractOnlyFileExtensionsFromContext = context =>
  extractOptionsFromContext(context).onlyFileExtensions;
...
const shouldConsiderFile = context => {
  const ignoreFileExtensions = extractIgnoreFileExtensionsFromContext(context);
  const onlyFileExtensions = extractOnlyFileExtensionsFromContext(context);
  const extension = context.getFilename().split('.')[1];
  if (onlyFileExtensions) {
    return extension && onlyFileExtensions.includes(extension)
  } else if (ignoreFileExtensions) {
    return !extension || !ignoreFileExtensions.includes(extension)
  } else {
    return true;
  }
};
...
module.exports = {
  ...
  create: (context) => {
    if (!shouldConsiderFile(context)) return {};
    ...
    return {
      ClassDeclaration: (dec) => {
        if (!shouldConsiderClass(dec, onlySubclasses, ignoreSubclasses)) return;
        const node = dec.body;
        const instanceMethods = extractInstanceMethodsFromBodyNode(node, ignoreMethodNames);
        const constructor = extractConstructorMethodFromBodyNode(node);
        if (!constructor && instanceMethods.length) {
          reportConstructor(context, dec);
        }
        verifyAndReport(context, instanceMethods, dec.id.name, constructor);
      }
    };
  }
};
